...
Images, Colors, ImageTransformations, Interpolations, GLAbstraction
...
function compile!(gl::GLData, vert_shader::String, frag_shader::String)
	...
	glShaderSource(vsh, read(vert_shader,String))
	glCompileShader(vsh)

	fsh = glCreateShader(GL_FRAGMENT_SHADER)
	glShaderSource(fsh, read(frag_shader,String))
	glCompileShader(fsh)

	gl.program = glCreateProgram()
	glAttachShader(gl.program, vsh)
	glAttachShader(gl.program, fsh)
